...
public struct PlayerSnapshot {
	public float RawHeadRotDegrees;
...
public class SM64Input(OnixSM64Config pluginConfig) {
	...
	public InputEvents UpdateInput(ISm64Mario mario) {
		...
		float playerYawRadians = -snap.RawHeadRotDegrees * (MathF.PI / 180f);
	...
	public string? BuildCameraCommand(ISm64Mario mario, Vector3 worldOffset) {
		...
		Vec3 marioWorldPos = marioPos + SM64CollisionWorld.ToVec3(worldOffset);
		Vec3 cameraTarget = marioWorldPos + new Vec3(0, 1f, 0);

		float yawRad = (-snap.RawHeadRotDegrees) * (MathF.PI / 180f);
		float pitchRad = snap.PitchDegrees * (MathF.PI / 180f);
		const float dist = 5f;
	...
	public (Vec3 lookFrom, Vec3 lookTo, Vec3 entityPos1, Vec3 entityPos2) BuildPunchVectors(
		...
		float pitch = -snap.PitchDegrees * (MathF.PI / 180f);
