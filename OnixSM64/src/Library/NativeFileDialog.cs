...
public static class NativeFileDialog {
	...
	public static string? ShowZ64FilePicker(IntPtr ownerHwnd = 0) {
		IFileOpenDialog dialog = (IFileOpenDialog)new FileOpenDialogCoClass();

		try {
			COMDLG_FILTERSPEC[] filters = [
				new() { pszName = "Nintendo 64 ROM (.z64)", pszSpec = "*.z64" },
			...
			dialog.SetTitle("Select a Nintendo 64 ROM (.z64)");
			dialog.SetDefaultExtension("z64");
